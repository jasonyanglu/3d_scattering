...
function filter = my_periodize_filter(filter_f)
	N = size(filter_f);
	N = N(N>1);
	...
	while 1
		...
		if length(N) == 1
			sz_in = [N/2^j0 2^j0 1 1 1 1];
			sz_out = [N/2^j0 1];
        elseif length(N) == 2
			sz_in = [N(1)/2^j0 2^j0 N(2)/2^j0 2^j0 1 1];
			sz_out = N./2^j0;
        else
		...
		filter.coefft{j0+1} = reshape( ...
			sum(sum(sum(reshape(filter_fj,sz_in),2),4),6),sz_out);
