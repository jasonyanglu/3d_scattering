...
function [x_phi, x_psi, meta_phi, meta_psi, options] = wavelet_2d_pyramid_double(x, filters, options)
    
    % check options white list
    if (~exist('options','var')), options = struct(); end
    white_list = {'J', 'origJ', 'precision', 'j_min', 'q_mask', 'all_low_pass'};
    ...
    h = filters.h.filter;
    for j = 1:options.J
        if options.J~=options.origJ || j~=1
            h.coefft = h.coefft2;
        end
        signal = hx{j};
        ...
        for j = options.j_min:options.J-1
            ...
            for pf = 1:numel(g) %% todo : find
                ...
                if (options.q_mask(q+1))
                    if options.J~=options.origJ
                        g{pf}.coefft = g{pf}.coefft2;
                    end
