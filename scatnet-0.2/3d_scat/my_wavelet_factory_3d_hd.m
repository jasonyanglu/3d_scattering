...
function [Wop, filters] = my_wavelet_factory_3d_hd(size_in, filt_opt, scat_opt)
    % Options
    ...
    white_list_filt = {'filter_type', 'precision', 'Q', 'J', 'L1', 'L2',...
    ...
    switch filt_opt.filter_type
        case 'morlet'
            ...
            filters = my_morlet_filter_bank_3d(size_in, filt_opt);
        case 'shannon'
            filt_opt = rmfield(filt_opt, 'filter_type');
	...
	for m = 1:scat_opt.M+1
		...
		Wop{m} = @(x)(my_wavelet_layer_3d_hd(x, filters, options_W));
    end
end