% MY_WAVELET_3D_PYRAMID Compute the scattering transform
%
% Usage
%   [x_phi, x_psi] = MY_WAVELET_3D_PYRAMID(x, filters, options)
%
% Input
%   x (numeric): the input signal
%   filters (struct): a 3d pyramid filter bank, typically obtained with
%       MY_MORLET_FILTER_BANK_3D_PYRAMID
...
function [x_phi, x_psi, meta_phi, meta_psi, options] = my_wavelet_3d_pyramid(x, filters, options)
    ...
    for j = 1:options.J
        ...
        signal_paded = my_pad_signal(signal, Npad, 'symm', 1);
        tmp = my_conv_sub_3d(signal_paded, h, 0);% modified here
        tmp = my_unpad_signal(tmp, 0, size(signal), filters.meta.offset);
        hx{j+1} = tmp;
    end
        ...
        for j = options.j_min:options.J-1
            ...
            for pf = 1:numel(g) %% todo : find
                ...
                if (options.q_mask(q+1))
                    tmp = my_conv_sub_3d(signal_paded, g{pf}, 0);
                    tmp = my_unpad_signal(tmp, 0, size(signal), filters.meta.offset);
                    x_psi{p} = tmp;
                    meta_psi.j(p) = j;
                    meta_psi.q(p) = q;
                    meta_psi.theta1(p) = filters.g.meta.theta1(pf);
                    meta_psi.theta2(p) = filters.g.meta.theta2(pf);
                    p = p+1;
                end
            end
        end
