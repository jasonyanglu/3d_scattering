...
function [Wop, filters, filters_rot] = wavelet_factory_3d_pyramid_nods(filt_opt, filt_rot_opt, scat_opt)
    if (nargin < 3)
        scat_opt = struct();
    end
    if (nargin < 2)
    ...
    Wop{1} = @(x)(wavelet_layer_2d_pyramid_nods(x, filters, wavelet_opt));
    Wop{2} = @(x)(wavelet_layer_3d_pyramid_nods(x, filters, filters_rot, wavelet_opt));
    Wop{3} = @(x)(wavelet_layer_3d_pyramid_nods(x, filters, filters_rot, wavelet_opt));
