...
function x_block = my_extract_block(x, nb_block)
    ...
    assert(mod(K,nb_block(3)) == 0);
	
	block_size = [N, M, K]./nb_block;
	x_block = reshape(permute(reshape(x,...
		block_size(1), nb_block(1), block_size(2), nb_block(2), block_size(3), nb_block(3)), [1 3 5 2 4 6]),...
		block_size(1), block_size(2), block_size(3), prod(nb_block));
end