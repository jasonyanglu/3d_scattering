%WAVELET_FACTORY_2D_PYRAMID_DOUBLE Summary of this function goes here
...
function [Wop, filters] = wavelet_factory_2d_pyramid_double(filt_opt, scat_opt)
	...
	switch (filt_opt.type)
        ...
        case 'double'
            filt_opt = rmfield(filt_opt, 'type');
            filters = double_density_filter_bank(filt_opt);
	end
	
	% wavelet transforms :
    wavelet_options = sub_options(scat_opt, {'J', 'precision', 'j_min', 'q_mask', 'all_low_pass'});
	for m = 1:scat_opt.M+1
		Wop{m} = @(x)(wavelet_layer_2d_pyramid_double(x, filters, wavelet_options));
	end
end
